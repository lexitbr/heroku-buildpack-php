...
function mkdep($require)
{
	return array_combine(array_map(function ($v) {
		return "heroku-sys/$v";
	}, array_keys($require)), $require);
}
// check if require section demands a runtime
function hasreq($require)
{
	return isset($require["php"]);
}
...
function mkmetas($package, array &$metapaks, &$have_runtime_req = false)
{
	...
	$platfilter = function ($v) {
		return preg_match("#^(php(-64bit)?$|ext-)#", $v);
	};
...
$require["heroku-sys/composer-plugin-api"] = "^" . explode(".", $lock["plugin-api-version"] ?? "1.0.0")[0]; # ... that supports the major plugin API version from the lock file - if the lock file says "2.99.0", we still want to select "^2", so the currently available Composer version can install lock files generated by brand new or pre-release Composer versions, as this stuff is generally forward compatible
